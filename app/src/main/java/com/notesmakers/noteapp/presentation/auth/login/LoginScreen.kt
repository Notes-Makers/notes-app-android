...
fun LoginScreen(
    ...
    LaunchedEffect(Unit) {
        loginViewModel.messageEvent.collect {
            when (it) {
                ...
                BaseViewModel.MessageEvent.Success -> snackbarHandler.showErrorSnackbar(message = "Jupi")
            }
    ...
    LoginScreen(navBack = {
        ...
        login = { loginViewModel.login("krystian.korbecki@gmail.com", "haslo123") })
}

@Composable
private fun LoginScreen(navBack: () -> Unit, login: () -> Unit) {
    ...
    Box {
        ...
        Column(
            ...
            FooterSignInScreen(onClick = {login()})
        }
    }
}
